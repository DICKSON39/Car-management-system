...
import { Loader2,MessageSquare } from "lucide-react"; // For loading states
...
export default function AdminBookings() {
  ...
  const { data: bookings, isLoading: isLoadingBookings } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bookings")
        .select("*, cars(name), drivers(name), profiles:user_id(full_name,email,phone)")
  ...
  const { data: drivers } = useQuery({
    queryKey: ["drivers"],
    queryFn: async () => {
      // Note: We might want to fetch ALL drivers here, 
      // otherwise currently assigned drivers might not show up in the list if they aren't 'available'
    ...
    {b.profiles?.phone && (
      <div className="flex items-center gap-2 mt-1">
        ...
        <Button
          ...
          onClick={() => {
            const message = `Hello ${b.profiles.full_name}, I'm following up on your booking for the ${b.cars?.name} (ID: ${b.id.split('-')[0].toUpperCase()}).`;
            window.open(`https://wa.me/${b.profiles.phone.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`, "_blank");
          }}
        >
          <MessageSquare className="h-3 w-3" />
                  ...
                  <TableCell>
                    <Select 
                      ...
                      value={b.status} 
                      onValueChange={(v) => updateBooking.mutate({ id: b.id, updates: { status: v } })}
                    >
                  ...
                  <TableCell className="text-right">
                    <Select 
                      ...
                      onValueChange={(v) => updateBooking.mutate({ id: b.id, updates: { driver_id: v === "none" ? null : v } })}
                    >
