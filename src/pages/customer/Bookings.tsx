...
export default function Bookings() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Review your past trips and manage invoices.
          </p>
        </div>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-6">
              <div className="space-y-4">
                ...
                <h2 className="text-2xl font-bold">
                  {(activeTrip as any).cars?.name}
                </h2>
                <div className="flex gap-4 text-sm text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <User className="h-4 w-4" />{" "}
                    {(activeTrip as any).drivers?.name}
                  </span>
                  <span className="flex items-center gap-1">
              ...
              <Button
                ...
                onClick={() =>
                  window.open(
                    `https://wa.me/${(activeTrip as any).drivers?.phone?.replace(/\D/g, "")}`,
                    "_blank",
                  )
                }
              >
              ...
              {bookings?.map((b) => (
                <TableRow key={b.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`${statusColors[b.status]} border-none font-bold text-[9px]`}
                    >
                  ...
                  <TableCell className="text-right">
                    {b.status === "completed" && !b.rating && (
                      <Button
                        size="sm"
                        variant="secondary"
                        className="h-7 text-[10px] bg-amber-100 text-amber-700 hover:bg-amber-200"
                      ...
                      <div className="flex justify-end items-center gap-2">
                        <div className="flex gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-3 w-3 ${i < b.rating ? "fill-amber-400 text-amber-400" : "text-gray-200"}`}
                            />
                          ))}
                        </div>
                        {b.review_text && (
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p className="max-w-[150px] text-[10px]">
                                {b.review_text}
                              </p>
                            </TooltipContent>
                          </Tooltip>
      ...
      <Dialog
        open={!!ratingBooking}
        onOpenChange={() => setRatingBooking(null)}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Trip Feedback</DialogTitle>
            <DialogDescription>
              How was your experience with the {ratingBooking?.cars?.name}?
            </DialogDescription>
          </DialogHeader>
